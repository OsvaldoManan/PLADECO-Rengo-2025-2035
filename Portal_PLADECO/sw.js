// ══════════════════════════════════════════════════════
// PLADECO Rengo 2025-2035 · Service Worker v69.33
...
// v69.31: v45.318 - OLA 3 #10 (consolidacion) RESUELTA COMO CROSS-LINKS, no como fusion. DIAGNOSTICO (mostrado al usuario antes de tocar, como exige el prompt): las secciones que el prompt maestro pedia FUSIONAR resultaron COMPLEMENTARIAS, no redundantes -> (a) los 4 FODA son 4 LENTES distintos con fuentes distintas: #foda comunal-tecnico (80 elementos), #fodaEntrev cualitativo-ciudadano (120+ entrevistas), FODA institucional en #diagnostico-institucional (904 menciones de funcionarios) y FODA por actor en #sociedad-civil; (b) los 2 comparadores son ESCALAS distintas: #comparador (Rengo vs comunas vecinas, que ADEMAS ya integra 'Rengo en O'Higgins' y ya es funcional con selector+tabla+insight => el objetivo del prompt ya estaba cumplido) vs #comparador-uv (unidades vecinales internas); (c) #etnografia (etnografia en profundidad) vs #voz (voz ciudadana amplia) son profundidad/fuente distintas; (d) los 3 modulos de seguridad tratan temas distintos (transito != delito != infraestructura vial). Fusionarlos habria BORRADO analisis y roto anclas/navegacion, violando 'no alterar contenido'. SOLUCION aditiva que cumple el objetivo (que dejen de leerse como repeticiones): componente 'Rutas de lectura' -> objeto EDITABLE window.seccionesRelacionadas (10 secciones, 25 enlaces) + renderSeccionesRelacionadas() que inyecta una franja .sec-rel (role=navigation, aria-label) al final de cada seccion, ANTES de su .sec-fuente, con tarjetas que explican POR QUE la hermana es complementaria ('la misma matriz desde la voz ciudadana', 'la misma logica hacia adentro', etc.). Navegacion via PV.go + scroll al ancla (soporta anclas internas como #fodaEntrev), con fallback al href normal si no hay motor de vistas. Idempotente, sin dependencias, sin temporizadores de UI. CSS .sec-rel theme-aware AA. VERIFICADO en navegador: 10 franjas inyectadas (foda 3, voz 3, etnografia 3, diagnostico-institucional 3, sociedad-civil 3, comparador 2, comparador-uv 2, seguridad-vial 2, seguridad-publica 2, movilidad-vial 2 = 25 tarjetas), todas antes de .sec-fuente; clic en tarjeta FODA -> #voz pasa a display:block y #foda a none (navegacion real); contraste AA/AAA en claro (#0f172a / #475569 sobre #f8fafc) y oscuro (#f1f5f9 / #cbd5e1 sobre #0a0f1c); sin scroll horizontal en 375px (doc 375=375, grid colapsa a 1 columna); consola sin errores propios (los 2 ERR_NAME_NOT_RESOLVED son CDN externos -leaflet/chart.js/CARTO- por el sandbox offline). Solo index.html + sw.js. NO se fusiono, movio ni elimino ninguna seccion; NO se alteraron datos. Cache bump v69.30 -> v69.31.
// v69.30: v45.317 - SEO (OLA 4, cierre): DEDUP de Open Graph/Twitter en conflicto. DIAGNOSTICO previo confirmo que la OLA 1 (contador/viewport/lazy) y casi toda la OLA 2-4 del 'prompt maestro' YA estaban hechas (countdown comentado L30858, #main-content centrado a 1440px v45.233, charts/maps lazy via cola SafeChart + IntersectionObserver, imagenes loading=lazy + width/height, sitemap.xml + robots.txt presentes, og/twitter/JSON-LD completos). El UNICO defecto SEO real: habia DOS sets de Open Graph/Twitter -el canonico (lineas ~21-39: og:url=/Portal_PLADECO/ + og-image.jpg, coherente con <link canonical> y el JSON-LD @graph GovernmentOrganization+WebSite+Article) y un DUPLICADO (lineas 334-348) que declaraba og:url a la RAIZ del repo y og:image=2.png-, lo que daba a los crawlers og:url/og:image ambiguos. Se elimino el set duplicado y se dejo solo el canonico (reemplazado por comentario explicativo). VERIFICADO: una sola og:url (/Portal_PLADECO/), un solo og:image/twitter:image (og-image.jpg). Solo index.html + sw.js (<head>, sin impacto en render/consola). NO se alteraron datos. PENDIENTE del prompt maestro: OLA 3 #10 (consolidar/fusionar modulos) es DESTRUCTIVO y el propio prompt exige 'mostrar que se fusiona antes de aplicar' -> queda como plan para validacion del usuario. Cache bump v69.29 -> v69.30.
// v69.29: v45.316 - SISTEMA DE DISENO · BLOQUE 2 (consistencia de tarjetas, cierre). HALLAZGO: B2 ya estaba mayormente hecho -el bloque 'DISTRIBUCION INTEGRAL v45.35' ya consolida .kpi/.censo-kpi/.mbht-kpi y .viz-card/.foda-card/.eje-card/.arg-card/.ict-scard/.diag-card a --rad-md + --surface-1 + --border-2 + --sh-card con padding y hover unificados-. Lo UNICO pendiente: extender el RADIO consistente (--rad-md, 12px) a las tarjetas con acento que quedaron fuera de esa lista (.coh-tipo-card/.coh-seq-card/.voz-nudo-card/.voz-nna-card/.ficha-puente-card/.fin-hall-card/.nna-card/.ineq-card/.anest-index-card/.anest-recom/.tr-card). SOLO border-radius -se PRESERVA su borde/acento de color, fondo y sombra propios- para no romper la semantica visual. Integra en la misma capa v45.35 (no duplica). VERIFICADO: .ficha-puente-card = 12px (= --rad-md); 0 errores en consola. Solo index.html + sw.js. NO se alteraron datos. NOTA de cierre del sistema de diseno: B3 (grillas/contenedores canonicos, .grid-auto-*/.grid-2/3/4) y la consolidacion tipografica YA existian en v45.35; B6 (emojis->SVG) se hizo en Fase 1.3; contraste de cuerpo AA en Fase 1.1; escaneabilidad (Lectura Facil + desplegables) ya presente. Lo genuinamente pendiente y de alto valor -paleta de graficos en tokens, ejes legibles, titulos sin degradado, radio de tarjetas- quedo cubierto en B1/B4/B5/B2 (v45.312-316). Cache bump v69.28 -> v69.29.
...
// ══════════════════════════════════════════════════════
const CACHE_STATIC='pladeco-static-v69.33';
const RELEASE='v45.320'; // version legible (user-facing), se muestra en el sello del footer
