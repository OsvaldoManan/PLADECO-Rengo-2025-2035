// ══════════════════════════════════════════════════════
// PLADECO Rengo 2025-2035 · Service Worker v69.69
// Estrategia: network-first HTML · stale-while-revalidate assets · imágenes precache-first→cache-first · tiles cache-first
// v69.69: v45.360 - MAPAS EN BUENAS CONDICIONES y TEXTO JUSTIFICADO (pedidos del encargado del PLADECO). MAPAS: rotulos de localidades sin choques (se muestran por orden de poblacion y se oculta el que chocaria con otro o con un control; al acercar reaparecen), legibles en tema oscuro (texto claro con halo oscuro); en telefonos la leyenda dentro del mapa ya no tapa el centro de Rengo: se pliega y un boton 'Ver leyenda' la abre y la cierra (aria-expanded); los rotulos de zona del catastro van al borde norte de su circulo y no tapan los puntos; el boton 'Perfil Zonal' se leia en blanco sobre blanco en tema oscuro. TEXTO JUSTIFICADO: todo el texto corrido (parrafos, listas, definiciones, citas y pies) se justifica con particion silabica; se respeta lo que esta centrado y no se aplica a controles ni a columnas de menos de 240 px, donde abriria huecos grandes. VERIFICADO: verificador de sintaxis en 0; en Edge sin interfaz con la pagina visible, los 8 mapas con todas sus teselas; en 'Explora tu territorio' 9 rotulos, 5 visibles a 1280 px y 4 a 390 px, sin choques entre ellos, y en 390 px la leyenda plegada con su boton; texto justificado en historia (36 de 37 parrafos visibles; el restante esta centrado), voz (30 de 31) y FODA (90 de 90), sin parrafos largos sin justificar; 0 elementos fuera del viewport en las 75 vistas a 360 y 1440 px; piezas del sistema grafico sin problemas a 5 anchos; 0 errores de pagina. Cache bump v69.68 -> v69.69.
// v69.68: v45.359 - PUBLICACION DE LA REVISION EDITORIAL (v45.343 a v45.359) y MAPAS VISIBLES. Se publica en main la rama revision-editorial. MAPAS: el fondo cartografico de CARTO paso a exigir llave de API y devolvia cada tesela con 'API KEY REQUIRED' impreso encima, asi que ningun mapa del sitio se podia leer; los nueve mapas usan ahora OpenStreetMap, sin llave y con su atribucion, con el color atenuado para que manden los datos e invertido en tema oscuro (OSM no tiene variante oscura). Leaflet 1.9.4 (BSD-2) y Chart.js 4.4.4 (MIT) quedan autoalojados en vendor/ con sus licencias: antes venian de unpkg y jsDelivr, y si el CDN tardaba o estaba bloqueado los mapas quedaban en blanco y ningun grafico se dibujaba. El mapa de 'Explora tu territorio' (#mapContainer) no se dibujaba ni en el sitio publicado ni en la rama: su arranque dependia del lote de graficos diferidos y se rendia a los 6 s; el bloque nuevo ARRANQUE DE LOS MAPAS llama al arranque de cada mapa al abrir su vista y recalcula su tamano. Sin conexion al servidor de teselas se dibuja el contorno comunal (archivo local) y el mapa lo avisa; si Leaflet no carga, el contenedor lo dice. La leyenda del catastro ya no tapa el zoom. v45.358: Poppins cargaba en la fuente del sistema porque sus @font-face estaban anidadas dentro de :root (invalidas); pasan al nivel superior y se sirven como subconjuntos latinos WOFF (618 KB a 60 KB). Boton de favorito cortado en 67 de 75 vistas a 360 px por la regla de area tactil de v45.351, y rejillas que tomaban el ancho de su contenido: 0 elementos fuera del viewport en las 75 vistas a 5 anchos (antes 166, 144 y 80 a 360, 390 y 768 px). Rotulos de las laminas medidos con la fuente real. Guiones de graficos/fuentes agregados (los excluia el .gitignore) con rutas relativas; reproducen byte a byte los archivos publicados. v45.356: grupos glosario (un solo mecanismo de siglas, sin tooltips anidados) y flotantes (superficies flotantes en AA, color-scheme por tema, avisos que se recuerdan, asistente sin tapar el aviso de instalacion). PENDIENTE: 8 grupos de la auditoria (documentos e imagenes, escalas y capas de mapas, indices y barra de navegacion, legibilidad, sistema visual, formatos) y la correccion del historial; decision municipal sobre los CSV con datos personales. VERIFICADO: verificador de sintaxis en 0; en Edge sin interfaz con la pagina en estado visible (sin eso no corren requestAnimationFrame ni IntersectionObserver y el contenido diferido no aparece), los 8 mapas presentes se dibujan al abrir su vista con todas sus teselas y capas a 1440 y 390 px, y con los servidores de teselas bloqueados aparecen el contorno y el aviso; 0 elementos fuera del viewport en las 75 vistas a 360, 390, 768, 1024 y 1440 px con Poppins cargada; los 74 botones de favorito dentro del viewport y sin tapar texto; contraste: 54 fallos de 16.808 textos en claro y 111 de 16.823 en oscuro, las muestras del barrido estan en secciones que este cambio no toco (contadores de ODS, etiquetas de #historia, pestanas de #etnografia, un enlace de descarga y botones del boletin); quedan pendientes; ninguna libreria se pide a unpkg ni a jsDelivr. Cache bump v69.67 -> v69.68.
...
// ══════════════════════════════════════════════════════
const CACHE_STATIC='pladeco-static-v69.69';
const RELEASE='v45.360'; // version legible (user-facing), se muestra en el sello del footer
